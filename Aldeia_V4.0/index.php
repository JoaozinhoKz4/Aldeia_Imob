            ...
            <div class="objects-search bg-dark dark">
                ...
                <div class="container">
                    ...
                    <form action="./">
                        ...
                        <div class="row align-items-end">
                            <div class="col-lg-3 col-md-4 form-group">
                               <label for="valor_imovel"><h5 class="mb-3"><i class="fa fa-dollar text-muted mr-2"></i>Valor</h5></label> 
                                <div class="form-inline">
...
//// Validação de qual o filtro estabelecido
    ...
    foreach($xml -> Imovel as $item){ 
        ...
        if ( utf8_decode($item -> Valorvenda) != ""){ $valor = utf8_decode($item -> Valorvenda);}
        if ( utf8_decode($item -> Valorlocacao) != ""){ $valor = utf8_decode($item -> Valorlocacao);}
        if ( utf8_decode($item -> Valortemporada) != ""){ $valor = utf8_decode($item -> Valortemporada);}
...
//// DEfinição das variáveis a serem enviadas para a página de cada anúncio
        ...
        if ( utf8_decode($item -> Numero) != "") {$numero = utf8_decode($item -> Numero);}
        else {$numero = NULL;}
        if ( utf8_decode($item -> Quadra) != "") {$quadra = utf8_decode($item -> Quadra);}
        else {$quadra = NULL;}
        if ( utf8_decode($item -> Regiao) != "") {$regiao = utf8_decode($item -> Regiao);}
        else {$regiao = NULL;}
        if ( utf8_decode($item -> Pontoreferencia) != "") {$ponto_ref = utf8_decode($item -> Pontoreferencia);}
        else {$ponto_ref = NULL;}
        if ( utf8_decode($item -> Areaterrenosemdeciamal) != ""){ $terreno = utf8_decode($item -> Areaterrenosemdeciamal);}
        else {$terreno = NULL;}
        if ( utf8_decode($item -> Areacosntruidasemdeciamal) != ""){$construida = utf8_decode($item -> Areacosntruidasemdeciamal);}
        else{$construida = NULL;}
        if ( utf8_decode($item -> Areautilsemdeciamal) != ""){$util = utf8_decode($item -> Areautilsemdeciamal);}
        else {$util = NULL;}
        if ( utf8_decode($item -> Banheiroauxiliar) != ""){$banheiro_a = utf8_decode($item -> Banheiroauxiliar);}
        else {$banheiro_a = NULL;}
        if ( utf8_decode($item -> Banheiroempregada) != ""){$banheiro_e = utf8_decode($item -> Banheiroempregada);}
        else {$banheiro_e = NULL;}
        if ( utf8_decode($item -> Banheiro) != ""){ $banheiro1 = utf8_decode($item -> Banheiro);}
        else {$banheiro1 = NULL;}
        if ( utf8_decode($item -> Banheiro2) != ""){ $banheiro2 = utf8_decode($item -> Banheiro2);}
        else {$banheiro2 = NULL;}
        if ( utf8_decode($item -> Dormitorios) != ""){ $dormitorio = utf8_decode($item -> Dormitorios);}
        else {$dormitorio = NULL;}
        if ( utf8_decode($item -> Valorvenda) != ""){$valor_venda = utf8_decode($item -> Valorvenda);}
        else {$valor_venda = NULL;}
        if ( utf8_decode($item -> Valorlocacao) != ""){$valor_locacao = utf8_decode($item -> Valorlocacao);}
        else {$valor_locacao = NULL;}
        if ( utf8_decode($item -> Valortemporada) != ""){$valor_temporada = utf8_decode($item -> Valortemporada);}
        else {$valor_temporada = NULL;}
        if ( utf8_decode($item -> Valorcondominio) != ""){$valor_condominio = utf8_decode($item -> Valorcondominio);}
        else {$valor_condominio = NULL;}
        if ( utf8_decode($item -> Valoriptu) != ""){$valor_iptu = utf8_decode($item -> Valoriptu);}
        else {$valor_iptu = NULL;}
